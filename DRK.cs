                    ...
                    if (InCombat())
                    {
                        ...
                        if (CanWeave(actionID))
                        {
                            ...
                            if (IsEnabled(CustomComboPreset.DRK_ManaOvercap))
                            {
                                ...
                                {
                                    if (IsEnabled(CustomComboPreset.DRK_EoSPooling) && GetCooldownRemainingTime(ÑªÂÒ) >= 40 && (gauge.HasDarkArts || LocalPlayer.CurrentMp > (mpRemaining + 3000)) && LevelChecked(EdgeOfDarkness) && CanDelayedWeave(actionID))
                            ...
                            if (gauge.DarksideTimeRemaining > 1)
                            {
                                ...
                                {
                                    if (IsEnabled(CustomComboPreset.DRK_BloodWeapon) && IsOffCooldown(ÊÈÑª) && LevelChecked(ÊÈÑª))
                                        return ÊÈÑª;

                                    if (IsEnabled(CustomComboPreset.DRK_Delirium) && IsOffCooldown(ÑªÂÒ) && LevelChecked(ÑªÂÒ) && (gauge.Blood <= 70 || GetBuffStacks(Buffs.BloodWeapon) < 0))
                                        return ÑªÂÒ;
                                }
                                ...
                                if (IsEnabled(CustomComboPreset.DRK_MainComboCDs_Group))
                                {
                                    ...
                                    {
                                        if ((GetRemainingCharges(Shadowbringer) > 0 && IsNotEnabled(CustomComboPreset.DRK_ShadowbringerBurst)) || (IsEnabled(CustomComboPreset.DRK_ShadowbringerBurst) && GetRemainingCharges(Shadowbringer) > 0 && gauge.ShadowTimeRemaining > 1 && IsOnCooldown(ÑªÂÒ))) //burst feature
                                    ...
                                    {
                                        if (IsNotEnabled(CustomComboPreset.DRK_MeleePlunge) || (IsEnabled(CustomComboPreset.DRK_MeleePlunge) && GetTargetDistance() <= 1 && ((GetMaxCharges(Plunge) > 1 && GetCooldownRemainingTime(ÑªÂÒ) >= 45) || GetMaxCharges(Plunge) == 1)))
                                            return Plunge;
                                    }
                                }
                            }
                        ...
                        //Delirium Features
                        if (LevelChecked(ÑªÂÒ) && IsEnabled(CustomComboPreset.DRK_Bloodspiller) && IsEnabled(CustomComboPreset.DRK_MainComboCDs_Group))
                        {

                            //¶ÔÍÅ¸¨
                            if (GetCooldownRemainingTime(LivingShadow) >= 40)
                            {
                                if (FindTargetEffectAny(RaidBuff.Á¬»·¼Æ) is not null || FindTargetEffectAny(RaidBuff.¹¥Æä²»±¸) is not null)
                                {
                            ...
                            // if (GetCooldownRemainingTime(ÑªÂÒ) >= 40 && GetCooldownRemainingTime(LivingShadow) is >= 40)
                            // {
                            //     if (gauge.Blood >= 50 || GetBuffStacks(Buffs.Delirium) > 0)
                            //     {
                            ...
                            // ²»ÓÃ¶ÔÍÅ¸¨Ö±½Ó·Å
                            if (GetCooldownRemainingTime(ÑªÂÒ) >= 40 && GetCooldownRemainingTime(LivingShadow) is >= 40 and <= 70)
                            {
                                if (gauge.Blood >= 50 || GetBuffStacks(Buffs.Delirium) > 0)
                                {
                                    // Dalamud.Logging.PluginLog.Error("1");
                            ...
                            //·ÀÖ¹°µÑªÒç³ö
                            if (comboTime > 0 && lastComboMove == SyphonStrike && gauge.Blood >= 60 && GetBuffStacks(Buffs.BloodWeapon) > 0 && GetBuffStacks(Buffs.Delirium) > 0)
                            {
                                // Dalamud.Logging.PluginLog.Error("1");
                                return Ñª½¦;
                            }
                            ...
                            if (gauge.Blood == 100 && (GetCooldownRemainingTime(ÊÈÑª) is >= 0 and < 5 || GetCooldownRemainingTime(ÑªÂÒ) is >= 0 and < 5))
                            {
                                return Ñª½¦;
                            }


                            //·ÀÖ¹ÂÒ´òÑª½¦£¬µ¼ÖÂ¸¥À×ÑÓºó
                            if (comboTime > 0 && lastComboMove == SyphonStrike && gauge.Blood >= 70 && GetCooldownRemainingTime(ÊÈÑª) is >= 0 and < 5 && GetCooldownRemainingTime(LivingShadow) is > 5)
                            {
                            ...
                            //·ÀÖ¹ÑÓºóÑªÂÒ
                            if (gauge.Blood >= 60 && GetCooldownRemainingTime(ÊÈÑª) is >= 0 and < 5 && GetCooldownRemainingTime(ÑªÂÒ) is >= 0 and < 5 && GetCooldownRemainingTime(LivingShadow) is > 7.5f)
                            {
                            ...
                            //·ÀÖ¹Ñª½¦Ã»ÓÐ´òÍê
                            // if (GetBuffStacks(Buffs.Delirium) > 0 && GetBuffRemainingTime(Buffs.Delirium)> 0 && GetBuffRemainingTime(Buffs.Delirium) < (7.5f + GetCooldownRemainingTime(Ñª½¦)))
                            if (GetBuffStacks(Buffs.Delirium) > 0 && GetCooldownRemainingTime(ÑªÂÒ) + GetCooldownRemainingTime(Ñª½¦) <= 50.2f)
                            {
                                // Dalamud.Logging.PluginLog.Error($"{GetCooldownRemainingTime(ÑªÂÒ)}  {GetBuffRemainingTime(Buffs.Delirium)} -> {7.5f + GetCooldownRemainingTime(Ñª½¦)}");
                                return Ñª½¦;
                            }
                            ...
                            //Delayed Delirium
                            if (IsEnabled(CustomComboPreset.DRK_DelayedBloodspiller) && GetBuffStacks(Buffs.Delirium) > 0 && IsOnCooldown(ÊÈÑª) && GetBuffStacks(Buffs.BloodWeapon) < 2)
                            {
                        ...
                        if (comboTime > 0)
                        {
                            ...
                            {
                                if (IsEnabled(CustomComboPreset.DRK_BloodGaugeOvercap) && LevelChecked(Ñª½¦) && gauge.Blood >= 90)
                                    return Ñª½¦;
                                return Souleater;
                            }
                        }
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == StalwartSoul)
                {
                    ...
                    if (CanWeave(actionID))
                    {
                        ...
                        if (gauge.DarksideTimeRemaining > 1)
                        {
                            if (IsEnabled(CustomComboPreset.DRK_AoE_BloodWeapon) && IsOffCooldown(ÊÈÑª) && LevelChecked(ÊÈÑª))
                                return ÊÈÑª;
                            if (IsEnabled(CustomComboPreset.DRK_AoE_Delirium) && IsOffCooldown(ÑªÂÒ) && LevelChecked(ÑªÂÒ))
                                return ÑªÂÒ;
                    ...
                    {
                        if (LevelChecked(ÑªÂÒ) && HasEffect(Buffs.Delirium) && gauge.DarksideTimeRemaining > 0)
                            return Quietus;
                    }
