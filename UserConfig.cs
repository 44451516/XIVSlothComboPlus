    ...
    public static class UserConfigItems
    {
        ...
        internal static void Draw(CustomComboPreset preset, bool enabled)
        {
            ...
            if (preset == CustomComboPreset.DNC_ST_Simple_SaberDance)
                UserConfig.DrawSliderInt(50, 100, DNC.Config.DNCSimpleSaberThreshold, "伶俐", 150, SliderIncrements.Fives);
            ...
            if (preset == CustomComboPreset.DNC_AoE_Simple_SaberDance)
                UserConfig.DrawSliderInt(50, 100, DNC.Config.DNCSimpleAoESaberThreshold, "伶俐", 150, SliderIncrements.Fives);
            ...
            {
                UserConfig.DrawHorizontalRadioButton(PLD.Config.PLD_AoE_RequiescatWeave, "好了就用", "Weaves Requiescat immediately after Fight or Flight for a standard burst window.", 0, 150, ImGuiColors.ParsedGreen);
                UserConfig.DrawHorizontalRadioButton(PLD.Config.PLD_AoE_RequiescatWeave, "GCD后半段", "在GCD后半段", 1, 150, ImGuiColors.DalamudYellow);
                ImGui.Spacing();
            }
            ...
            {
                UserConfig.DrawHorizontalRadioButton(PLD.Config.PLD_ST_RequiescatWeave, "好了就用", "Weaves Requiescat immediately after Fight or Flight for a standard burst window.", 0, 150, ImGuiColors.ParsedGreen);
                UserConfig.DrawHorizontalRadioButton(PLD.Config.PLD_ST_RequiescatWeave, "GCD后半段", "在GCD后半段", 1, 150, ImGuiColors.DalamudYellow);
                ImGui.Spacing();
            }
            ...
            if (preset == CustomComboPreset.PLD_ST_AdvancedMode_Atonement)
            {
                ...
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_AtonementTiming, "能用就用[推荐]", "在重新启动基本组合之前使用赎罪。", 1);;
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_AtonementTiming, "快溢出了使用", "在基本连招结束前使用赎罪。", 2);
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_AtonementTiming, "备用", "前面两种的组合。", 3);
            }
            
            if (preset == CustomComboPreset.PLD_ST_AdvancedMode_HolySpirit)
            {
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_DivineMightTiming, "能用就用", "有了就用", 1);
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_DivineMightTiming, "快溢出了使用", "下一个快好了用", 2);
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_DivineMightTiming, "备用", "前面两种的组合", 3);
                UserConfig.DrawRadioButton(PLD.Config.PLD_ST_DivineMightTiming, "智能模式[推荐]", "智能模式", 4);
            ...
            if (preset == CustomComboPreset.SMN_DemiEgiMenu_oGCDPooling)
            {
                ...
                UserConfig.DrawHorizontalRadioButton(SMN.Config.SMN_BurstPhase, "巴哈姆特或菲尼克斯3", "巴哈或凤凰阶段爆发 (取决于那个先到).", 3);
